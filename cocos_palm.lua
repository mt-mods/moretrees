...
local trunk = core.registered_nodes["moretrees:palm_trunk"]
local ftrunk = {}
local gftrunk = {}
...
ftrunk.after_destruct = function(pos, oldnode)
	local coconuts = core.find_nodes_in_area(
		{ x = pos.x - 1, y = pos.y, z = pos.z - 1 },
		{ x = pos.x + 1, y = pos.y, z = pos.z + 1 },
		{ "group:moretrees_coconut" }
	)
	for _, coconutpos in pairs(coconuts) do
		-- core.dig_node(coconutpos) does not cause nearby coconuts to be dropped :-( ...
		--core.dig_node(coconutpos)
		local items = core.get_node_drops(core.get_node(coconutpos).name)
		core.swap_node(coconutpos, { name = "air" })
		for _, itemname in pairs(items) do
			core.add_item(coconutpos, itemname)
		end
	end
end
...
core.register_node("moretrees:palm_fruit_trunk", ftrunk)
core.register_node("moretrees:palm_fruit_trunk_gen", gftrunk)
...
local coconut_regrow_abm_spec = {
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if math.random(2 ^ #coconuts) <= 2 then
			...
			local coconutnode = core.get_node(coconutpos)
			if coconutnode.name == "air" then
				core.swap_node(coconutpos, { name = "moretrees:coconut_0" })
			end
		end
	end,
}
if moretrees.coconuts_regrow then
	core.register_abm(coconut_regrow_abm_spec)
end
...
core.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		core.swap_node(pos, { name = "moretrees:palm_fruit_trunk" })
		local poslist = core.find_nodes_in_area(
			{ x = pos.x - 1, y = pos.y, z = pos.z - 1 },
			{ x = pos.x + 1, y = pos.y, z = pos.z + 1 },
			"air"
		)
		...
		for _, gen in pairs(genlist) do
			core.swap_node(gen.pos, { name = "moretrees:coconut_3" })
			count = count + 1
			if count == 4 then
				break
			end
...
local coconut_growfn = function(pos, elapsed)
	...
	elseif not moretrees.coconuts_regrow then
		...
		core.swap_node(pos, { name = "moretrees:coconut_3" })
		return
...
for _, suffix in ipairs({ "_0", "_1", "_2", "_3", "" }) do
	...
	core.register_node("moretrees:coconut" .. suffix, coconutdef)
end
...
if moretrees.coconuts_convert_existing_palms then
	local spec = {
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			trunks = core.find_nodes_in_area(
				{ x = pos.x - 1, y = pos.y, z = pos.z - 1 },
				{ x = pos.x + 1, y = pos.y, z = pos.z + 1 },
			...
			cvtrunks = core.find_nodes_in_area(
			...
			leaves = core.find_nodes_in_area(
				{ x = tpos.x - 1, y = tpos.y + 1, z = tpos.z - 1 },
				{ x = tpos.x + 1, y = tpos.y + 1, z = tpos.z + 1 },
			...
			for _, tpos_1 in pairs(cvtrunks) do
				core.swap_node(tpos_1, { name = "moretrees:palm_fruit_trunk" })
				coconuts = core.find_nodes_in_area(
					{ x = tpos_1.x - 1, y = tpos_1.y, z = tpos_1.z - 1 },
					{ x = tpos_1.x + 1, y = tpos_1.y, z = tpos_1.z + 1 },
					"moretrees:coconut"
				)
				for _, coconutpos in pairs(coconuts) do
					core.swap_node(coconutpos, { name = "moretrees:coconut_3" })
				end
			end
		end,
	}
