...
core.register_craftitem("moretrees:coconut_milk", {
	...
	if itemstack:get_name() == "moretrees:coconut_milk" then
		for i, j in pairs(old_craft_grid) do
			...
			if (ocg_name ~= "") and (ocg_name ~= "moretrees:coconut") and (ocg_name ~= "vessels:drinking_glass") then
				...
				if core.get_item_group(ocg_name, "food_cutting_board") == 1 then
					return
				end
...
if core.registered_items["farming:cutting_board"] then
	core.register_craft({
		type = "shapeless",
		output = "moretrees:coconut_milk",
		recipe = {
...
local flour
if core.registered_nodes["farming:flour"] then
	flour = "farming:flour"
else
